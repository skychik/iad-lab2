function validate() {
    ...
    if (!x.value) {
        //alert ("Set X");
        document.getElementById("Xwarning").innerHTML = "Set X";
        fl = false;
    }
    if (!isFinite(parseFloat(+ x.value.replace(',', '.')))) {
        //alert("X is not a real number");
        document.getElementById("Xwarning").innerHTML = "X is not a real number";
        fl = false;
    } else if (parseFloat(+ x.value.replace(',', '.')) < -5.0 || parseFloat(+ x.value.replace(',', '.')) > 5.0) {
        //alert("X gotta be between -5 and 5");
        document.getElementById("Xwarning").innerHTML = "X gotta be between -5 and 5";
        fl = false;
    }
    ...
    if (!y.value || y.value === "" || y.value === "choose y") {
        //alert ("Choose Y");
        document.getElementById("Ywarning").innerHTML = "Choose Y";
        fl = false;
    }

    if (!r.value || r.value === "" || r.value === "choose r") {
        //alert ("Choose R");
        document.getElementById("Rwarning").innerHTML = "Choose R";
        fl = false;
    }
